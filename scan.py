...
def process_site(site):
    ...
    cmd = [cwd / "amass", "enum", "-active", "-df", site, "-dir", cwd / "amassdb", "-o", new]
...
if __name__ == '__main__':
    ...
    header_args = ["-H", f"Authorization: Bearer {RUMBLE_API_TOKEN}", "-H", "Content-Type: application/json"]
    ...
    sites = cwd.glob("inputs/*.domains.txt")
    # Run up to 8 enumerations in parallel
    with Pool(8) as p:
        print(p.map(process_site, sites))
