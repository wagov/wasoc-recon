...
if (
    rumble_sites_file.exists()
    and time() - rumble_sites_file.stat().st_mtime < 60 * 60 * 24
):
    print(f"Skipping rumble exports as updated in last 24 hours")
else:
...
for site in cwd.glob("inputs/*.domains.txt"):
    ...
    if (
        subdomains.exists()
        and time() - subdomains.stat().st_mtime < 60 * 60 * 48
        and site.stat().st_mtime < subdomains.stat().st_mtime
    ):
        print(f"Skipping {name} as updated in last 48 hours")
    else:
